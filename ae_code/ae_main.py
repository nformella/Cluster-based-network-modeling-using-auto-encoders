...
Example:
    ...
    neurons_per_layer = [[50,10,50], [10], [100,30,20,30,100]]
...
import math
from ae_and_train import build_model, DenseAe, prep_training, train_ae
...
neurons_per_layer = [[50,10,50]]
activations = [[pt.tanh, pt.tanh, pt.tanh, pt.selu],
]
batch_size = [100, 90]
learning_rate = [1e-4]
...
count_models = 0
for paras in itertools.product(neurons_per_layer, activations, learning_rate, 
                                                            batch_size):
    
    # Preselection: Number of activation functions must match number of layers
    if len(paras[0]) == len(paras[1])-1:
        count_models += 1
        model, device = build_model(X, *paras[0:2])
        ...
        loss_values = train_ae(model, epochs, train_loader, datapnts_at_t, 
                                ...
                                print_reconLoss=print_reconLoss)

        aemodels.append(copy.deepcopy(model)) 
        loss_lists.append((loss_values))

if count_models == 0:
    for paras in itertools.product(neurons_per_layer, activations, 
...
parameters_str = []
for i, paras in enumerate(itertools.product(neurons_per_layer, activations, 
                                                learning_rate, batch_size)):
    ...
    if len(paras[0]) == len(paras[1])-1:
        ...
        ax.plot(loss_lists[i], label=legend_name, marker=next(marker), ms=5, 
                                                                markevery=100)
        ax.legend()
